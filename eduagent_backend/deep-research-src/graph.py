...
from nodes.kg_write import kg_write
from nodes.citation_connector import citation_connector
...
) -> Literal["critique", "citation_connector"]:
    """After executive summary, go to critique if more iterations remain; else run A8."""
    configurable = Configuration.from_runnable_config(config)
    iteration = state.get("iteration", 0)
    # iterations=1 → no critique; iterations=N → N-1 critique cycles
    if iteration < configurable.research_iterations - 1:
        return "critique"
    return "citation_connector"
...
builder.add_node("citation_connector", citation_connector)
builder.add_node("final_report_generation", final_report)
builder.add_node("qa_audit", qa_audit)
builder.add_node("kg_write", kg_write)
...
builder.add_conditional_edges(
    ...
    {
        "critique": "critique",
        "citation_connector": "citation_connector",
    },
)
# After critique → back to supervisor for the next iteration
builder.add_edge("critique", "research_supervisor")
# A8 runs once, feeds into final report
builder.add_edge("citation_connector", "final_report_generation")
# qa_audit and kg_write run in parallel after the report is generated
builder.add_edge("final_report_generation", "qa_audit")
builder.add_edge("final_report_generation", "kg_write")
builder.add_edge("qa_audit", END)
builder.add_edge("kg_write", END)
