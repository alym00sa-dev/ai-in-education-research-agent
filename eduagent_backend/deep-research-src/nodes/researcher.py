...
async def researcher(state: ResearcherState, config: RunnableConfig) -> dict:
    ...
    db_names = ", ".join(tn for tn, _ in sweep_plan)
    run_logger.log(f"[researcher] Sweep cycle={cycle} | topic='{research_topic[:60]}' | DBs: {db_names}", sid)
    ...
    async def run_sweep_entry(tool_name: str, query_field: str):
        ...
        try:
            run_logger.log(f"[researcher] → calling {effective_tool_name} | q='{query[:60]}'", sid)
            ...
            run_logger.log(f"[researcher] ✓ {effective_tool_name} returned {len(result_str)} chars", sid)
    ...
    for tool_name, query_field, raw_output in raw_results:
        ...
        if configurable.enable_pdf_extraction and tool_name in PDF_EXTRACTABLE_TOOLS:
            output, profiles = await enrich_tool_output(
                ...
                tool_output=output,
                research_topic=research_topic,
                metadata_model=configurable.model,
                taxonomy_model=configurable.model,
            )
...
async def researcher_reflect(state: ResearcherState, config: RunnableConfig) -> dict:
    ...
    run_logger.log(f"[researcher_reflect] topic='{research_topic[:60]}' | decision={result.decision}", sid)
...
async def compress_research(state: ResearcherState, config: RunnableConfig) -> dict:
    ...
    messages = state.get("researcher_messages", [])
    research_topic = state.get("research_topic", "")
    run_logger.log(f"[compress_research] Compressing findings for '{research_topic[:60]}'", sid)
