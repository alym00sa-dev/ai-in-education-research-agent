...
async def supervisor_tools_node(
    ...
    if active_calls:
        ...
        for _tc in active_calls:
            _args = _tc["args"]
            run_logger.log(f"[supervisor] Dispatching researcher — tier={_args.get('tier','?')} | {_args.get('research_topic','')[:80]}", configurable.session_id)
        ...
        for tc, result in results:
            ...
            topic_label = tc["args"].get("research_topic", "")[:60]
            profiles_n = len(result.get("paper_profiles", []))
            cycles_n = result.get("sweep_cycles", 0)
            run_logger.log(f"[supervisor] Researcher done — '{topic_label}' | sweep_cycles={cycles_n} | profiles={profiles_n}", configurable.session_id)
            tool_messages.append(ToolMessage(
