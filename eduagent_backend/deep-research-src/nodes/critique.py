...
async def critique(state: AgentState, config: RunnableConfig) -> dict:
    ...
    run_logger.log(f"[researcher] {len(all_notes)} total notes — {len(paper_profiles)} paper profiles", configurable.session_id)
    run_logger.log(f"[critique] Iteration {iteration} — evaluating {len(paper_profiles)} profiles, {len(all_notes)} notes", configurable.session_id)
    ...
    run_logger.log(f"[critique] Iteration {iteration + 1} critique ready.", configurable.session_id)
