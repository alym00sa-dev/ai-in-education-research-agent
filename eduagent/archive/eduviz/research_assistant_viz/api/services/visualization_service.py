...
class VisualizationService:
    ...
    # ========== P5: DELIVERY PILLAR (ADAPTIVE INSTRUCTION & TUTORING) ==========

    def get_p5_data(self) -> Dict[str, Any]:
        """
        Get data for P5: Delivery Pillar visualizations.
        Returns geographic, demographic, institution, and grade level distributions over time.
        Filters for Adaptive Instruction & Tutoring interventions only (high-quality RCTs).
        """
        import csv
        import json
        from collections import defaultdict

        # Load adaptive instruction intervention IDs
        base_path = Path(__file__).parent.parent.parent
        mapping_path = base_path / 'wwc_level3_mapped.json'

        with open(mapping_path, 'r') as f:
            mapped = json.load(f)
        adaptive_ids = {item['intervention_id'] for item in mapped
                       if item['implementation_objective'] == 'Adaptive Instruction & Tutoring Systems'}

        # Process CSV data grouped by year
        csv_path = base_path.parent / 'kg-viz-frontend' / 'WWC Analysis' / 'Interventions_Studies_And_Findings.csv'

        data_by_year = defaultdict(lambda: {
            'studies': {},  # study_id -> {size, intervention_id, intervention_name, ...}
            'states': defaultdict(int),  # state -> student_count
            'interventions_by_state': defaultdict(lambda: defaultdict(int)),  # state -> intervention -> count
            'demographics_by_state': defaultdict(lambda: {'FRPL': 0, 'ELL': 0, 'IEP': 0, 'Minority': 0}),  # state -> demo -> count
            'institutions_by_state': defaultdict(lambda: {'Public': 0, 'Charter': 0, 'Private': 0, 'Parochial': 0}),  # state -> inst_type -> count
            'grade_levels_by_state': defaultdict(lambda: {'PK': 0, 'K-5': 0, '6-8': 0, '9-12': 0})  # state -> grade -> count
        })
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                # Filter for adaptive instruction interventions
                if row['i_InterventionID'] not in adaptive_ids:
                    continue

                # Filter: Meets WWC standards (with or without reservations) + RCT
                rating = row.get('s_Study_Rating', '')
                if rating not in ['Meets WWC standards without reservations', 'Meets WWC standards with reservations']:
                    continue
                if 'randomized' not in row.get('s_Study_Design', '').lower():
                    continue

                # Get publication year
                pub_year = row.get('s_Publication_Date', '')
                if not pub_year or not pub_year.strip():
                    continue
                ...
                study_id = row.get('s_StudyID', '')
                if not study_id:
                    continue

                # Get sample size
                size_str = row.get('f_Outcome_Sample_Size', '0')
                try:
                    sample_size = int(float(size_str)) if size_str and size_str.strip() else 0
                except:
                    sample_size = 0
                ...
                intervention_name = row.get('i_Intervention_Name', 'Unknown')

                year_data = data_by_year[year]

                # Track unique studies (take max sample size per study to avoid double-counting)
                if study_id not in year_data['studies'] or sample_size > year_data['studies'][study_id]['size']:
                    year_data['studies'][study_id] = {
                        'size': sample_size,
                        'intervention_id': intervention_id,
                        'intervention_name': intervention_name
                    }

                # Geographic data (intervention-level aggregated)
                state_cols = [col for col in reader.fieldnames
                             if col.startswith('i_Region_State_')
                             and col not in ['i_Region_State_Midwest', 'i_Region_State_Northeast',
                                           'i_Region_State_South', 'i_Region_State_West']]

                # Count how many states this study covers
                states_covered = []
                for state_col in state_cols:
                    val = row.get(state_col, '')
                    if val and val.strip() and float(val) > 0:
                        state_name = state_col.replace('i_Region_State_', '').replace('_', ' ')
                        states_covered.append(state_name)

                # Distribute sample size evenly across states to avoid inflation
                if states_covered:
                    size_per_state = sample_size / len(states_covered)
                    for state_name in states_covered:
                        year_data['states'][state_name] += size_per_state
                        year_data['interventions_by_state'][state_name][intervention_name] += size_per_state

                # Demographics - distribute by state
                # Intervention-level percentages
                frpl = row.get('i_Demographics_Sample_FRPL', '')
                ell = row.get('i_Demographics_Sample_ELL', '')

                # Study-level percentages
                iep = row.get('s_Disability_IEP', '')
                minority = row.get('s_Demographics_Sample_Minority', '')

                if states_covered:
                    size_per_state = sample_size / len(states_covered)

                    for state_name in states_covered:
                        # FRPL (intervention-level)
                        if frpl and frpl.strip():
                            try:
                                year_data['demographics_by_state'][state_name]['FRPL'] += size_per_state * (float(frpl) / 100)
                            except:
                                pass

                        # ELL (intervention-level)
                        if ell and ell.strip():
                            try:
                                year_data['demographics_by_state'][state_name]['ELL'] += size_per_state * (float(ell) / 100)
                            except:
                                pass

                        # IEP (study-level)
                        if iep and iep.strip():
                            try:
                                year_data['demographics_by_state'][state_name]['IEP'] += size_per_state * (float(iep) / 100)
                            except:
                                pass

                        # Minority (study-level)
                        if minority and minority.strip():
                            try:
                                year_data['demographics_by_state'][state_name]['Minority'] += size_per_state * (float(minority) / 100)
                            except:
                                pass

                        # Helper function to check if a field is "true" (any numeric value > 0)
                        def is_true(val):
                        ...
                        # Institution types - distribute count by state
                        if is_true(row.get('i_School_type_Public')):
                            year_data['institutions_by_state'][state_name]['Public'] += 1 / len(states_covered)
                        if is_true(row.get('i_School_type_Charter')):
                            year_data['institutions_by_state'][state_name]['Charter'] += 1 / len(states_covered)
                        if is_true(row.get('i_School_type_Private')):
                            year_data['institutions_by_state'][state_name]['Private'] += 1 / len(states_covered)
                        if is_true(row.get('i_School_type_Parochial')):
                            year_data['institutions_by_state'][state_name]['Parochial'] += 1 / len(states_covered)

                        # Grade levels - distribute students by state
                        if is_true(row.get('i_Grade_PK')):
                            year_data['grade_levels_by_state'][state_name]['PK'] += size_per_state

                        has_k5 = False
                        for grade in ['K', '1', '2', '3', '4', '5']:
                            if is_true(row.get(f'i_Grade_{grade}')):
                                has_k5 = True
                                break
                        if has_k5:
                            year_data['grade_levels_by_state'][state_name]['K-5'] += size_per_state
                        ...
                        for grade in ['6', '7', '8']:
                            if is_true(row.get(f'i_Grade_{grade}')):
                                ...
                                break
                        if has_68:
                            year_data['grade_levels_by_state'][state_name]['6-8'] += size_per_state
                        ...
                        for grade in ['9', '10', '11', '12']:
                            if is_true(row.get(f'i_Grade_{grade}')):
                                ...
                                break
                        if has_912:
                            year_data['grade_levels_by_state'][state_name]['9-12'] += size_per_state

        # Build cumulative time slices
        ...
        cumulative_data = {
            ...
            'interventions_by_state': defaultdict(lambda: defaultdict(int)),
            'demographics_by_state': defaultdict(lambda: {'FRPL': 0, 'ELL': 0, 'IEP': 0, 'Minority': 0}),
            'institutions_by_state': defaultdict(lambda: {'Public': 0, 'Charter': 0, 'Private': 0, 'Parochial': 0}),
            'grade_levels_by_state': defaultdict(lambda: {'PK': 0, 'K-5': 0, '6-8': 0, '9-12': 0})
        }

        for year in all_years:
            year_data = data_by_year[year]

            # Update cumulative studies
            cumulative_data['studies'].update(year_data['studies'])

            # Update cumulative geographic data
            for state, count in year_data['states'].items():
                cumulative_data['states'][state] += count

            for state, interventions in year_data['interventions_by_state'].items():
                for intervention, count in interventions.items():
                    cumulative_data['interventions_by_state'][state][intervention] += count

            # Update cumulative demographics by state
            for state, demographics in year_data['demographics_by_state'].items():
                for demo, count in demographics.items():
                    cumulative_data['demographics_by_state'][state][demo] += count

            # Update cumulative institutions by state
            for state, institutions in year_data['institutions_by_state'].items():
                for inst, count in institutions.items():
                    cumulative_data['institutions_by_state'][state][inst] += count

            # Update cumulative grade levels by state
            for state, grade_levels in year_data['grade_levels_by_state'].items():
                for grade, count in grade_levels.items():
                    cumulative_data['grade_levels_by_state'][state][grade] += count

            # Calculate totals
            total_students = sum(study['size'] for study in cumulative_data['studies'].values())
            total_studies = len(cumulative_data['studies'])

            # Build geographic data
            geographic_data = []
            for state, student_count in cumulative_data['states'].items():
                # Round intervention counts to integers
                interventions_dict = {k: int(round(v)) for k, v in cumulative_data['interventions_by_state'][state].items()}
                geographic_data.append({
                    'state': state,
                    'student_count': int(round(student_count)),
                    'study_count': len([s for s in cumulative_data['studies'].values()]),  # approximation
                    'interventions': interventions_dict
                })

            # Build demographic data by state
            demographic_data = []
            for state, demographics in cumulative_data['demographics_by_state'].items():
                state_total = sum(demographics.values())
                if state_total > 0:  # Only include states with demographic data
                    for category, count in demographics.items():
                        if count > 0:  # Only include categories with data
                            ...
                            demographic_data.append({
                                'state': state,
                                'category': category,
                                'student_count': int(round(count)),
                                'percentage': round(percentage, 2)
                            })

            # Build institution data by state
            institution_data = []
            for state, institutions in cumulative_data['institutions_by_state'].items():
                state_total = sum(institutions.values())
                if state_total > 0:  # Only include states with institution data
                    for inst_type, count in institutions.items():
            ...
            # Build grade level data by state
            grade_level_data = []
            for state, grade_levels in cumulative_data['grade_levels_by_state'].items():
                state_total = sum(grade_levels.values())
                if state_total > 0:  # Only include states with grade level data
                    for grade, count in grade_levels.items():
                        if count > 0:  # Only include grades with data
                            percentage = (count / state_total * 100) if state_total > 0 else 0
        ...
        metadata = {
            'pillar': 'P1: Core Instruction & Tutoring (Adaptive)',
            'description': 'Geographic, demographic, and institutional distribution of high-quality RCT evidence for adaptive instruction and tutoring systems',
            'icon_scale': {
                'person': '1 icon = 100 students',
                'building': '1 icon = 1 institution'
            },
            'filters_applied': [
                'Implementation Objective: Adaptive Instruction & Tutoring Systems',
                'Study Rating: Meets WWC standards (with or without reservations)',
    ...
    # ========== P1: EFFECT SIZE OVER TIME ==========

    def get_p1_data(self) -> Dict[str, Any]:
        """
        ...
        # Load intervention to use_case mapping from WWC Analysis folder
        mapping_path = base_path.parent / 'kg-viz-frontend' / 'WWC Analysis' / 'intervention_use_case_mapping.json'
        with open(mapping_path, 'r') as f:
            mapping_data = json.load(f)

        # Load wwc_level3_mapped to get intervention IDs for Adaptive Instruction & Tutoring Systems
        wwc_mapped_path = base_path / 'wwc_level3_mapped.json'
        with open(wwc_mapped_path, 'r') as f:
            wwc_mapped = json.load(f)

        # Get intervention IDs for Adaptive Instruction & Tutoring Systems
        adaptive_intervention_ids = {item['intervention_id'] for item in wwc_mapped
                                     if item['implementation_objective'] == 'Adaptive Instruction & Tutoring Systems'}

        # Create intervention name -> use_case mapping (exclude "unmatched")
        # This maps the canonical intervention names to use cases
        intervention_name_to_usecase = {}
        for item in mapping_data['interventions']:
            use_case = item['use_case']
            # Skip interventions marked as "unmatched"
            if use_case != 'unmatched':
                intervention_name_to_usecase[item['intervention_name']] = use_case

        # Create intervention_id -> intervention_name mapping from wwc_mapped
        intervention_id_to_name = {}
        for item in wwc_mapped:
            if item['intervention_id'] in adaptive_intervention_ids:
                intervention_id_to_name[item['intervention_id']] = item['intervention_name']

        # Load WWC CSV data
        csv_path = base_path.parent / 'kg-viz-frontend' / 'WWC Analysis' / 'Interventions_Studies_And_Findings.csv'

        # Data structures for aggregation
        # by_intervention: intervention_name -> year -> list of findings
        # by_usecase: use_case -> year -> list of findings
        by_intervention = defaultdict(lambda: defaultdict(list))
        by_usecase = defaultdict(lambda: defaultdict(list))

        with open(csv_path, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)

            for row in reader:
                # Use intervention-level ID (same as P5) not finding-level name
                intervention_id = row.get('i_InterventionID', '').strip()

                # Only process Adaptive Instruction & Tutoring Systems interventions
                if intervention_id not in adaptive_intervention_ids:
                    continue

                # Get the canonical intervention name
                intervention_name = intervention_id_to_name.get(intervention_id)
                if not intervention_name:
                    continue

                # Only process mapped interventions (excludes unmatched)
                if intervention_name not in intervention_name_to_usecase:
                    continue

                # Apply same quality filters as P5: Meets WWC standards + RCT
                rating = row.get('s_Study_Rating', '')
                if rating not in ['Meets WWC standards without reservations', 'Meets WWC standards with reservations']:
                    continue

                study_design = row.get('s_Study_Design', '').lower()
                if 'randomized' not in study_design:
                    continue

                # Extract data
                try:
                    year = int(row.get('s_Publication_Date', '0') or '0')
                    if year == 0:
                        continue

                    effect_size_str = row.get('f_Effect_Size_WWC', '').strip()
                    if not effect_size_str:
                        continue
                    effect_size = float(effect_size_str)

                    sample_size_str = row.get('f_Outcome_Sample_Size', '0').strip() or '0'
                    sample_size = int(float(sample_size_str))

                    is_significant = row.get('f_Is_Statistically_Significant', 'False').strip()

                    direction = row.get('f_FavorableUnfavorableDesignation', '').strip()

                    # Infer direction from effect size if not provided
                    if not direction:
                        if effect_size > 0.05:
                            direction = 'Favorable'
                            ...
                            direction = 'No discernable effect size direction'

                    # Get comparison group info (for multi-arm/multi-site trials)
                    intervention_mean_str = row.get('f_Intervention_Mean', '').strip()
                    comparison_mean_str = row.get('f_Comparison_Mean', '').strip()
                    intervention_mean = float(intervention_mean_str) if intervention_mean_str else None
                    comparison_mean = float(comparison_mean_str) if comparison_mean_str else None

                    # Get cluster counts (for cluster-randomized trials showing # of schools/sites)
                    comparison_clusters_str = row.get('f_Comparison_Clusters_SS', '').strip()
                    comparison_clusters = int(comparison_clusters_str) if comparison_clusters_str else None

                    finding_data = {
                        'effect_size': effect_size,
                        'sample_size': sample_size,
                        'is_significant': is_significant == 'True',
                        'direction': direction,  # Favorable, Unfavorable, No discernable effect size direction
                        'study_id': row.get('s_StudyID', '').strip(),
                        'study_citation': row.get('s_Citation', '').strip(),
                        'outcome_measure': row.get('f_Outcome_Measure', '').strip(),
                        'outcome_domain': row.get('f_Outcome_Domain', '').strip(),
                        'period': row.get('f_Period', '').strip(),
                        'sample_description': row.get('f_Sample_Description', '').strip(),
                        'is_subgroup': row.get('f_Is_Subgroup', 'False').strip() == 'True',
                        'intervention_mean': intervention_mean,  # Mean score for intervention group
                        'comparison_mean': comparison_mean,  # Mean score for comparison/control group
                        'comparison_clusters': comparison_clusters,  # Number of schools/sites in control group
                        'finding_id': row.get('f_FindingID', '').strip(),
                        'intervention_name': intervention_name  # Add intervention name for use-case grouping
                    }

                    # Add to by_intervention
                    by_intervention[intervention_name][year].append(finding_data)

                    # Add to by_usecase
                    use_case = intervention_name_to_usecase[intervention_name]
                    by_usecase[use_case][year].append(finding_data)

                except (ValueError, TypeError) as e:
                    # Skip rows with invalid data
                    continue

        # Process by_intervention data into time series
        intervention_series = []
        use_case_colors = {
            'math_tutoring': '#3b82f6',  # blue
            'automated_grading': '#10b981',  # green
            'real_time_feedback': '#f59e0b',  # amber
            'instructional_planning': '#8b5cf6',  # purple
            'teacher_coaching': '#ec4899'  # pink
        }

        # By Intervention view removed - too many interventions (40) to display clearly
        # Keeping only By Use-case view below

        # Process by_usecase data into time series
        usecase_series = []
        for use_case in sorted(by_usecase.keys()):
            years_data = by_usecase[use_case]
            data_points = []

            # Track global max sample size per study across ALL years
            global_study_samples = {}
            studies_seen_in_previous_years = set()

            for year in sorted(years_data.keys()):
                findings = years_data[year]

                total_effect = sum(f['effect_size'] for f in findings)
                avg_effect_size = total_effect / len(findings) if findings else 0

                # Get max sample size per study in this year (deduplicates multi-arm studies)
                year_study_samples = {}
                for f in findings:
                    study_id = f['study_id']
                    if study_id:
                        year_study_samples[study_id] = max(
                            year_study_samples.get(study_id, 0),
                            f['sample_size']
                        )

                # Update global max sample size for each study
                for study_id, sample_size in year_study_samples.items():
                    global_study_samples[study_id] = max(
                        global_study_samples.get(study_id, 0),
                        sample_size
                    )

                # Cumulative students = sum of all study maxes seen so far
                cumulative_students = sum(global_study_samples.values())

                # New students = only studies appearing for the FIRST time in this use case
                new_studies_this_year = set(year_study_samples.keys()) - studies_seen_in_previous_years
                new_students = sum(year_study_samples[sid] for sid in new_studies_this_year)

                # Update the set of studies seen so far
                studies_seen_in_previous_years.update(year_study_samples.keys())

                # Determine dominant direction
                direction_counts = defaultdict(int)
                for f in findings:
                    direction_counts[f['direction']] += 1

                dominant_direction = max(direction_counts.items(), key=lambda x: x[1])[0] if direction_counts else 'No discernable effect size direction'

                # Compile study details - deduplicate by study_id
                studies = []
                studies_seen_in_year = set()
                for f in findings:
                    if f['study_id'] and f['study_id'] not in studies_seen_in_year:
                        studies_seen_in_year.add(f['study_id'])
                        study_findings = [sf for sf in findings if sf['study_id'] == f['study_id']]
                        # Use MAX sample size across all findings for this study (not sum - avoids double-counting)
                        study_max_sample = max(sf['sample_size'] for sf in study_findings)
                        studies.append({
                            'study_id': f['study_id'],
                            'citation': f['study_citation'],
                            'sample_size': study_max_sample,
                            'intervention_name': f['intervention_name'],  # Add intervention name for use-case view
                            'findings': [{
                                'effect_size': sf['effect_size'],
                                'outcome_measure': sf['outcome_measure'],
                                'outcome_domain': sf['outcome_domain'],  # Domain category
                                'period': sf['period'],  # Timing (immediate, follow-up)
                                'sample_description': sf['sample_description'],  # Sample/subgroup info
                                'is_subgroup': sf['is_subgroup'],  # Boolean for subgroup analysis
                                'intervention_mean': sf['intervention_mean'],  # Intervention group mean score
                                'comparison_mean': sf['comparison_mean'],  # Control group mean score
                                'comparison_clusters': sf['comparison_clusters'],  # Number of schools/sites in control
                                'is_significant': sf['is_significant'],
                                'direction': sf['direction']
                            } for sf in study_findings]
                        })

                data_points.append({
                    'year': year,
                    'effect_size': round(avg_effect_size, 3),
            ...
            usecase_series.append({
                'id': use_case,
                'label': use_case.replace('_', ' ').title(),
                'color': use_case_colors.get(use_case, '#6b7280'),
                'data_points': data_points
            })
        ...
        metadata = {
            'x_axis': {
                'label': 'Year',
                'description': 'Publication year of study (individual years, not bucketed)',
                'computation': 'Each data point represents findings published in that specific year'
            },
            'y_axis': {
                'label': 'Effect Size',
                'description': 'Average WWC-calculated effect size for findings in that year',
                'computation': 'Mean of all effect sizes reported for the intervention/use-case in that year'
            },
            'bubble_size': {
                'label': 'Total Students',
                'description': 'Number of students from all studies with findings in that year',
                'computation': 'Sum of sample sizes across all findings for that intervention/use-case in that year'
            },
            'bubble_color': {
                'label': 'Dominant Finding Direction',
                'description': 'Overall direction of findings in that year',
                'values': {
                    'Favorable': 'Positive effects (green)',
                    'Unfavorable': 'Negative effects (red)',
                    'No discernable effect size direction': 'Mixed/neutral (grey)'
                }
            }
        }
    ...
    def _generate_distinct_colors(self, count: int) -> List[str]:
        ...
        for i in range(count):
            hue = i / count
            # Use varied saturation and lightness for better distinction
            saturation = 0.6 + (i % 3) * 0.15
            lightness = 0.45 + (i % 2) * 0.1
    ...
    def get_gates_investment_overlap_data(self):
        """Get Gates Foundation pre-LLM investment overlap with WWC study distribution."""
        import pandas as pd
        from pathlib import Path

        base_path = Path(__file__).parent

        # Load Gates investment data
        investment_path = base_path.parent.parent.parent / 'kg-viz-frontend' / 'INVESTMENT [Sensitive]' / 'aly_moosa_usp_invest_pre_LLM_1_2_full.xlsx'
        df_invest = pd.read_excel(investment_path)

        # Filter for Core Instruction & Tutoring (Tag_Category_1 = '[1]')
        filtered = df_invest[df_invest['Tag_Category_1'] == '[1]']
        ...
        # Extract state-specific investments
        state_investments = {}
        unallocated_amount = 0

        for _, row in filtered.iterrows():
            amount = row['Investment_Amount']
            geo = str(row['Geographies']) if pd.notna(row['Geographies']) else ''

            # Find states mentioned
            states_found = [state for state in us_states if state in geo]

            if states_found:
                per_state = amount / len(states_found)
                for state in states_found:
                    state_investments[state] = state_investments.get(state, 0) + per_state
            else:
                unallocated_amount += amount

        # Don't distribute unallocated investment - keep it separate
        # Build response with state data (only state-specific investments)
        state_data = []
        for state in us_states:
            investment = state_investments.get(state, 0)

            # Only include states with investment
            if investment > 0:
                state_data.append({
                    'state': state,
        ...
        state_data.sort(key=lambda x: x['investment_amount'], reverse=True)

        # Get WWC study distribution data (student estimates by state) from P5
        # Process CSV data for all years to get cumulative student estimates
        csv_path = base_path.parent.parent.parent / 'kg-viz-frontend' / 'WWC Analysis' / 'Interventions_Studies_And_Findings.csv'

        # Load adaptive instruction intervention IDs (same as P5)
        import json
        mapping_path = base_path.parent.parent / 'wwc_level3_mapped.json'
        ...
        # Process CSV for student distribution by state (same filters as P5)
        state_student_data = {}
        import csv
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            # Get intervention-level state columns (same as P5)
            state_cols = [col for col in reader.fieldnames
                         if col.startswith('i_Region_State_')
                         and col not in ['i_Region_State_Midwest', 'i_Region_State_Northeast',
                                       'i_Region_State_South', 'i_Region_State_West']]
            ...
            for row in reader:
                intervention_id = row.get('i_InterventionID', '').strip()
                if intervention_id not in adaptive_ids:
                    continue
                ...
                # Get sample size
                try:
                    sample_size = float(row.get('f_Outcome_Sample_Size', 0) or 0)
                except:
                    sample_size = 0

                if sample_size > 0:
                    # Find states covered by this intervention
                    states_covered = []
                    for state_col in state_cols:
                        val = row.get(state_col, '')
                        if val and val.strip() and float(val) > 0:
                    ...
                    if states_covered:
                        size_per_state = sample_size / len(states_covered)
                        for state_name in states_covered:
                            if state_name not in state_student_data:
                                state_student_data[state_name] = {'student_count': 0, 'study_count': 0}
                            state_student_data[state_name]['student_count'] += size_per_state
                            # Note: study_count is approximate since one study can span multiple states
                            state_student_data[state_name]['study_count'] += 1 / len(states_covered)

        # Build WWC distribution data
        wwc_distribution = []
        for state, data_dict in state_student_data.items():
            wwc_distribution.append({
                'state': state,
                'student_count': round(data_dict['student_count'], 0),
                'study_count': round(data_dict['study_count'], 0)
            })

        wwc_distribution.sort(key=lambda x: x['student_count'], reverse=True)

        return {
            'state_data': state_data,
            'wwc_distribution': wwc_distribution,
            'total_investment': round(filtered['Investment_Amount'].sum(), 2),
            'state_allocated_investment': round(sum(state_investments.values()), 2),
            'unallocated_investment': round(unallocated_amount, 2),
            'states_with_specific_investment': len(state_investments),
            'metadata': {
                'title': 'Gates Investment Pre-LLM Overlap',
                'description': 'State-specific breakdown of Gates Foundation pre-LLM investments in Core Instruction & Tutoring',
                'total_investments': len(filtered),
                'color_scale': 'Light green to dark green gradient based on state-specific investment amount'
            }
        }

    def get_p1_current_data(self, implementation_objective: str) -> Dict:
        """Generate P1Current Evidence Ladder visualization data.

        Args:
            implementation_objective: The implementation objective to filter by

        Returns:
            Dict containing rungs with classified papers
        """
        from src.neo4j_config import get_neo4j_connection

        conn = get_neo4j_connection()

        # Query all non-WWC papers for this implementation objective
        query = """
        MATCH (p:Paper)-[:HAS_IMPLEMENTATION_OBJECTIVE]->(io:ImplementationObjective {id: $io})
        WHERE p.source <> 'WWC' OR p.source IS NULL
        OPTIONAL MATCH (p)-[:FOCUSES_ON_OUTCOME]->(o:Outcome)
        WITH p, COLLECT(DISTINCT o.id) as outcomes
        RETURN p.title as title,
               p.url as url,
               p.year as year,
               p.study_design as study_design,
               p.population as population,
               p.study_size as study_size,
               p.results_summary as results_summary,
               outcomes
        ORDER BY p.year DESC
        """

        papers = conn.execute_query(query, {'io': implementation_objective})

        # Define the 6 evidence ladder rungs
        rungs_def = [
            {
                'rung_number': 1,
                'rung_name': 'Monitoring',
                'description': 'Usage studies, feasibility assessments, and single-site pilots exploring whether the intervention can be implemented',
                'papers': []
            },
            {
                'rung_number': 2,
                'rung_name': 'Implementation',
                'description': 'Implementation studies with fidelity measures documenting how the intervention is being used in practice',
                'papers': []
            },
            {
                'rung_number': 3,
                'rung_name': 'Comparative',
                'description': 'Quasi-experimental studies comparing intervention groups to control/comparison groups',
                'papers': []
            },
            {
                'rung_number': 4,
                'rung_name': 'Causal Effectiveness',
                'description': 'Single-site randomized controlled trials establishing causal effects in controlled settings',
                'papers': []
            },
            {
                'rung_number': 5,
                'rung_name': 'Generalizability',
                'description': 'Multi-site RCTs establishing effects across diverse contexts',
                'papers': []
            },
            {
                'rung_number': 6,
                'rung_name': 'Personalized',
                'description': 'Studies with subgroup analyses and heterogeneity of treatment effects showing who benefits most',
                'papers': []
            }
        ]

        # Classify each paper into a rung
        meta_analysis_count = 0
        papers_classified = 0

        for paper in papers:
            study_design = (paper.get('study_design') or '').strip()
            results_summary = (paper.get('results_summary') or '').lower()
            ...
            if study_design == 'Meta-Analysis/Systematic Review':
                # Meta-analyses are not included in the ladder
                meta_analysis_count += 1
                continue

            papers_classified += 1

            if study_design == 'Randomized Control Trial':
                # Check if it has subgroup analysis (Rung 6) or is multi-site
                if any(keyword in results_summary for keyword in ['subgroup', 'heterogen', 'moderator', 'differential']):
                    rungs_def[5]['papers'].append(paper_obj)
                elif any(keyword in results_summary for keyword in ['multi-site', 'multisite', 'multiple sites', 'across sites']):
                    rungs_def[4]['papers'].append(paper_obj)
                else:
                    # Default to Rung 4: Single-site RCT
                    rungs_def[3]['papers'].append(paper_obj)
            elif study_design == 'Quasi-Experimental Design':
                # Rung 3: Quasi-experimental
                rungs_def[2]['papers'].append(paper_obj)
            elif study_design == 'Mixed-Methods Study':
                # Check if implementation-focused (Rung 2) or exploratory (Rung 1)
                if any(keyword in results_summary for keyword in ['implementation', 'fidelity', 'adherence', 'dosage']):
                    rungs_def[1]['papers'].append(paper_obj)
                else:
                    rungs_def[0]['papers'].append(paper_obj)
            elif study_design == 'Qualitative Study':
                # Rung 1: Qualitative/exploratory
                rungs_def[0]['papers'].append(paper_obj)
            else:
                # No design specified or other - default to Rung 1
                rungs_def[0]['papers'].append(paper_obj)

        # Add paper counts to each rung
        for rung in rungs_def:
            rung['paper_count'] = len(rung['papers'])
        ...
        return {
            ...
            'metadata': {
                'title': f'Evidence Ladder: {implementation_objective}',
                'description': 'Distribution of research evidence across 6 rungs from monitoring to personalized effectiveness',
                'classifications': {
                    'Rung 1': ['Qualitative Study', 'Mixed-Methods (exploratory)', 'No design specified'],
                    'Rung 2': ['Mixed-Methods (implementation-focused)'],
                    'Rung 3': ['Quasi-Experimental Design'],
                    'Rung 4': ['Randomized Control Trial (single-site)'],
                    'Rung 5': ['Randomized Control Trial (multi-site)'],
                    'Rung 6': ['Studies with subgroup/heterogeneity analysis']
                }
            }
        }

    def _classify_to_rung(self, study_design: str, results_summary: str) -> int:
        """Helper to classify a paper to a rung index (0-5)."""
        if study_design == 'Randomized Control Trial':
            if any(keyword in results_summary for keyword in ['subgroup', 'heterogen', 'moderator', 'differential']):
                return 5  # Rung 6
            elif any(keyword in results_summary for keyword in ['multi-site', 'multisite', 'multiple sites', 'across sites']):
            ...
            return 0  # Rung 1 (default)

    def get_p1_current_by_usecase(self, implementation_objective: str) -> Dict:
        """Generate P1Current Evidence Ladder visualization data grouped by use case.

        Args:
            implementation_objective: The implementation objective to filter by

        Returns:
            Dict containing evidence ladders for each of 5 use cases
        """
        from src.neo4j_config import get_neo4j_connection
        ...
        # Define use case keywords for classification
        use_case_keywords = {
            'math_tutoring': ['math', 'mathematics', 'algebra', 'geometry', 'arithmetic', 'numeracy', 'calculation'],
            'automated_grading': ['grading', 'assessment', 'scoring', 'evaluation', 'automated feedback', 'auto-grade'],
            'real_time_feedback': ['feedback', 'immediate', 'real-time', 'adaptive', 'responsive', 'formative'],
            'instructional_planning': ['curriculum', 'lesson plan', 'instructional design', 'planning', 'pedagogy', 'teaching strategy'],
            'teacher_coaching': ['teacher training', 'professional development', 'coaching', 'teacher support', 'pd', 'educator']
        }
        ...
        use_case_labels = {
            ...
            'teacher_coaching': 'Teacher Coaching'
        }

        # Define the 6 evidence ladder rungs with descriptions
        rung_definitions = [
            {
                'rung_number': 1,
                'rung_name': 'Monitoring',
        ...
        # Initialize use case ladders
        use_case_ladders = {}
        for use_case_id, label in use_case_labels.items():
            use_case_ladders[use_case_id] = {
                ...
                'rungs': [
                    ...
                    for rung in rung_definitions
                ]
            }

        # Classify and add papers to use case ladders
        for paper in papers:
            study_design = (paper.get('study_design') or '').strip()
            ...
            results_lower = (paper.get('results_summary') or '').lower()
            outcomes_lower = ' '.join(paper.get('outcomes', [])).lower()

            # Classify paper into use case based on keywords
            best_match = None
            max_score = 0

            for use_case_id, keywords in use_case_keywords.items():
                score = sum(1 for keyword in keywords if keyword in title_lower or keyword in results_lower or keyword in outcomes_lower)
                if score > max_score:
                    max_score = score
                    best_match = use_case_id

            # If no keyword match, default to real_time_feedback (largest category)
            if not best_match:
                best_match = 'real_time_feedback'
            ...
            # Determine rung based on study design
            rung_index = self._classify_to_rung(study_design, results_lower)
            use_case_ladders[best_match]['rungs'][rung_index]['papers'].append(paper_obj)

        # Add paper counts to each rung in each use case
        for use_case_data in use_case_ladders.values():
            for rung in use_case_data['rungs']:
                rung['paper_count'] = len(rung['papers'])

        return {
            'use_case_ladders': list(use_case_ladders.values()),
            'total_papers': len(papers),
            'implementation_objective': implementation_objective,
            'metadata': {
                'title': f'Evidence Ladders by Use Case: {implementation_objective}',
                'description': 'Distribution of research evidence by use case across 6 evidence rungs',
                'use_cases': list(use_case_labels.values())
            }
        }
