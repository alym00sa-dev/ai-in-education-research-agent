...
const LEVEL_STYLE: Record<string, { color: string; prefix: string }> = {
  node:       { color: "#94a3b8", prefix: "--" },
  tool:       { color: "#818cf8", prefix: ">>" },
  researcher: { color: "#38bdf8", prefix: "->" },
  done:       { color: "#4ade80", prefix: "ok" },
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4" style={{ background: "#0d1117" }}>
              <div className="font-mono text-xs space-y-1.5">
                ...
                <div className="mt-1" style={{ color: "#484f58" }}>...</div>
              </div>
              <div ref={bottomRef} />
            </div>
          </>
...
function TerminalLine({ line }: { line: StatusLine }) {
  const style = LEVEL_STYLE[line.level] ?? LEVEL_STYLE.node;
  const textColor = TEXT_COLOR[line.level] ?? "#cbd5e1";
  return (
    <div className="flex items-start gap-2 leading-relaxed">
      <span className="flex-shrink-0 w-9 tabular-nums text-[10px] pt-px" style={{ color: "#484f58" }}>
        {formatMs(line.timestamp)}
      </span>
      <span className="flex-shrink-0 w-4 text-center" style={{ color: style.color }}>
        {style.prefix}
      </span>
